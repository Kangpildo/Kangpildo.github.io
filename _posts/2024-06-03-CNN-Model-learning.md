...
## 개요
이 프로젝트는 VGG-16 알고리즘을 활용하여 CNN 모델을 학습하고, 이미지 분류 문제를 해결하는 내용을 다룹니다. ㈜심스리얼리티의 데이터를 사용하여 3명의 얼굴 이미지를 분류하는 모델을 구현했습니다

## 데이터 설명
데이터 수집: 총 3명의 얼굴 사진을 수집하여 사용. 각 인당 학습 데이터 100장, 테스트 데이터 20장으로 총 360장의 이미지를 사용.
데이터 전처리: 얼굴 각도 변화가 적은 이미지를 사용. 이미지 사이즈를 동일하게 전처리하지 못하였으며, 정면 얼굴만을 사용하지 않음.
